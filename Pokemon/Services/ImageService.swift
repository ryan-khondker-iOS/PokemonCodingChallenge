...
final class ImageService {
    /**
        ...
        and pass the image to the completion block. If we get an error when fetching
        the image data, then we pass the error to the completion block
        - parameter imageUrl: the URL of the image
        - parameter session: the URL session to use to fetch the image
